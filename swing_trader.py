...
def trade_stock_symbol(symbol):
    ...
    try:
        ...
        if should_enter:
            ...
            if position_size > 0:  # Alpaca supports fractional shares for stocks
                place_order(symbol, OrderSide.BUY, position_size, f"STOCK ENTRY - {reason}")
            else:
                logger.info(f"⚠️ [{symbol}] Position size too small: {position_size}")
        else:
            logger.debug(f"⏳ [{symbol}] No entry signal - {reason}")
...
def manage_position_with_indicators(symbol, current_price, symbol_type, df):
    ...
    try:
        # Fix position detection - Alpaca throws exception when no position exists
        try:
            position = trading_client.get_position(symbol)
        except Exception:
            # No position exists - this is normal, not an error
            logger.debug(f"🔍 [{symbol}] No position found")
        ...
        # Log current position status with more detail
        logger.info(f"📊 [{symbol}] Position found - Entry: ${entry_price:.2f}, Current: ${current_price:.2f}, P&L: {pnl_percent:.2%}, Qty: {current_qty}")
        
        # REVISED PRIORITY: Check profit-taking FIRST, then technical exits
        # This ensures we don't miss profit opportunities due to technical signal delays
        
        # PRIORITY 1: Check stop-loss/take-profit/trailing stops
        profit_action, profit_reason = manage_position(symbol, current_price, symbol_type)
        
        if profit_action:
            logger.info(f"💰 [{symbol}] {profit_reason}")
            return True, profit_reason
        
        # PRIORITY 2: Check technical exit signals (only if no profit action taken)
        ...
        # Position held
        logger.debug(f"📈 [{symbol}] Position held - no exit conditions met")
...
def manage_position(symbol, current_price, symbol_type):
    ...
    try:
        # Fix position detection - same as above
        try:
            position = trading_client.get_position(symbol)
        except Exception:
            # No position exists
        ...
        current_qty = float(position.qty)
            ...
            # Check for scale-out opportunities with better validation
            for level_idx, (profit_threshold, sell_portion) in enumerate(scale_out_levels):
                ...
                if pnl_percent >= profit_threshold and level_key not in tracking['scale_out_executed']:
                    logger.info(f"🎯 [{symbol}] Scale-out trigger: {pnl_percent:.2%} >= {profit_threshold:.2%}")
                    
                    # Calculate quantity to sell (portion of current position)
                    qty_to_sell = abs(current_qty) * sell_portion
                    
                    # Round appropriately with better validation
                    if symbol_type == "crypto":
                        qty_to_sell = round(qty_to_sell, 6)  # Crypto allows fractional
                        min_qty = 0.000001  # Minimum crypto quantity
                    else:
                        qty_to_sell = max(1, int(qty_to_sell))  # Stocks need whole shares
                        min_qty = 1  # Minimum stock quantity
                    
                    logger.info(f"📊 [{symbol}] Scale-out calculation: {abs(current_qty)} * {sell_portion} = {qty_to_sell}")
                    
                    # Enhanced validation - ensure we have enough quantity and meet minimums
                    remaining_qty = abs(current_qty) - qty_to_sell
                    
                    # For the final scale-out level, allow selling the entire remaining position
                    is_final_level = level_idx == len(scale_out_levels) - 1
                    remaining_valid = remaining_qty >= min_qty or is_final_level
                    
                    if (qty_to_sell >= min_qty and 
                        qty_to_sell <= abs(current_qty) and 
                        remaining_valid):  # Ensure remaining position is valid or final exit
                        
                        place_order(symbol, OrderSide.SELL if current_qty > 0 else OrderSide.BUY,
                                   qty_to_sell, f"[P4] SCALE OUT {int(sell_portion*100)}% at {pnl_percent:.2%} profit")
                        tracking['scale_out_executed'].append(level_key)
                        logger.info(f"💰 [{symbol}] ✅ Scaled out {qty_to_sell} units at {pnl_percent:.2%} profit")
                        
                        # Don't delete tracking - position still open
                        return True, f"Partial profit taken at {pnl_percent:.2%}"
                    else:
                        logger.warning(f"⚠️ [{symbol}] Scale-out skipped: qty_to_sell={qty_to_sell}, current_qty={abs(current_qty)}, min_qty={min_qty}")
            
            # Full take profit check (for remaining position or if scaling not triggered)
            if pnl_percent >= take_profit_threshold:
                logger.info(f"🎯 [{symbol}] Full take profit trigger: {pnl_percent:.2%} >= {take_profit_threshold:.2%}")
                place_order(symbol, OrderSide.SELL if current_qty > 0 else OrderSide.BUY, 
                           abs(current_qty), f"[P4] TAKE PROFIT (final) at {pnl_percent:.2%}")
                if position_id in manage_position.trailing_data:
                    del manage_position.trailing_data[position_id]  # Clean up tracking
                return True, f"Full take profit at {pnl_percent:.2%}"
            
            # Enhanced debugging - show why no action was taken
            logger.debug(f"🔍 [{symbol}] Profit analysis:")
            logger.debug(f"    P&L: {pnl_percent:.2%} (need {take_profit_threshold:.2%} for full TP)")
            logger.debug(f"    Trailing stop: ${tracking['trailing_stop']:.2f if tracking['trailing_stop'] else 'None'}")
            logger.debug(f"    Scale-out executed: {tracking.get('scale_out_executed', [])}")
